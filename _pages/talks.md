---
layout: page
title: talks
permalink: /talks/
description: a list of talks I have given 
nav: true
nav_order: 2
---

##### A Survey on Deep Learning Approaches for Text-to-SQL 
*Presentation @ [[IMSI](https://www.athenarc.gr/en/imsi) Talks '24]* <!-- // Slides -->

---

##### Natural Language Interfaces for Databases with Deep Learning 
*Tutorial @ [VLDB '23](https://vldb.org/2023/) // [Slides](/assets/pdf/nlidbs_vld23.pdf)*

---

##### Data Democratisation with Deep Learning: The Anatomy of a Natural Language Data Interface 
*Tutorial @ [WSDM '23](https://www.wsdm-conference.org/2023/) // [Slides](/assets/pdf/datadem_wsdm23.pdf)*

---

##### Data Democratisation with Deep Learning: An Analysis of Text-to-SQL Systems 
*Tutorial @ [WWW '22](https://archives.iw3c2.org/www2022/) // [Slides](/assets/pdf/text2sql_twc22.pdf)*

---

##### A Deep Dive into Deep Learning Approaches for Text-to-SQL Systems 
*Tutorial @ [SIGMOD '21](https://2021.sigmod.org/) // [Slides](/assets/pdf/text2sql_sigmod21.pdf)*

---

##### Deep Learning Approaches for Text-to-SQL Systems
*Tutorial @ [EDBT '21](https://edbticdt2021.cs.ucy.ac.cy/index.html) // [Slides](/assets/pdf/text2sql_edbt21.pdf)*